...
export function BottomNav({ unreadAlerts = 0, unreadMessages = 0 }: { unreadAlerts?: number; unreadMessages?: number }) {
    ...
    <nav className="sticky bottom-0 z-30 mx-auto flex w-full max-w-md items-stretch gap-1 border-t border-line bg-card px-2 pb-[max(env(safe-area-inset-bottom),10px)] pt-2 shadow-[0_-4px_20px_rgba(42,31,21,0.06)]">
      {ITEMS.map((it) => {
        ...
        const badge =
          ...
          (it.href === "/messages" && unreadMessages > 0);
        const badgeCount = it.href === "/alerts" ? unreadAlerts : unreadMessages;
        return (
          <Link
            key={it.href}
            href={it.href}
