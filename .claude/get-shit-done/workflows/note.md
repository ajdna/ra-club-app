...
- **Global scope**: `D:/RA Club/CLUB APP/club-app/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/RA Club/CLUB APP/club-app/.claude/notes/`)
...
2. Glob `D:/RA Club/CLUB APP/club-app/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/RA Club/CLUB APP/club-app/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/RA Club/CLUB APP/club-app/.claude/notes/` — works in any directory
