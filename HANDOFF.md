...
- **Scheduler = Supabase `pg_cron` + `pg_net`** (job `club-dispatch`, `*/15`, calls `/api/cron/dispatch` with `Bearer <CRON_SECRET>`). GitHub Actions schedule was throttled (~hourly) → disabled (manual `workflow_dispatch` kept). Gotchas learned: header needs `Bearer ` prefix; `CRON_SECRET` must match Vercel env AND Vercel must be **redeployed** after any env change; verify via `net._http_response` (want 200). Vercel masks the secret — to rotate, overwrite in Vercel + redeploy + re-`cron.schedule` with same value.
...
- **2C-six** daily club session details (links change daily) — `club_sessions` table (date+period), Admin → Club sessions editor (leaders; paste full invite, `extractUrl` saves the link), in-app page `/club/[period]` (today's details + Join Zoom button), club notification deep-links to `/club/morning|evening` (internal, no 404). Reviewed GREEN, MERGING. Owner sets morning the eve before / evening same-day. (Old `club_schedule.morning_link/evening_link` now unused.)
